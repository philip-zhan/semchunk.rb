...
Rake::Task["release"].enhance do
  ...
  system "open https://github.com/philip-zhan/semchunk.rb/releases"
end
...
task :verify_gemspec_files do
  ...
  gemspec_files = Gem::Specification.load("semchunk.gemspec").files.sort
  ignored_by_git = gemspec_files - git_files
  next if ignored_by_git.empty?

  raise <<~ERROR

    The `spec.files` specified in semchunk.gemspec include the following files
...
namespace :bump do
  ...
  task :ruby do
    replace_in_file "semchunk.gemspec", /ruby_version = .*">= (.*)"/ => RubyVersions.lowest
